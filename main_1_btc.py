...
untrade1= os.path.abspath(r'C:\Users\harsh\Desktop\inter-iit\untrade-sdk')
        ...
        for index, row in data.iterrows():
            if row['signals'] == 1:  # long signal
                if trade_s == False and trade_l == False:
                    ...
                    data.at[index, 'trade'] = 'not_in_trade'  # exit short, enter long
                ...
                if trade_s == False and trade_l == False:
                    ...
                    data.at[index, 'trade'] = 'not_in_trade'  # exit long, enter short
                ...
                if trade_l == True:
                    data.at[index, 'trade'] = 'not_in_trade'
                    trade_l = False  # reset long flag
                ...
                if trade_s == True:
                    ...
                    data.at[index, 'trade'] = 'not_in_trade'  # not in trade
            ...
            for index, row in df.iterrows():
                if row['signals'] == 1:
                    if not trade_s and not trade_l:
                        ...
                        df.at[index, 'trade'] = 'not_in_trade'
                        trade_s = False
                ...
                for index, row in df.iterrows():
                    ...
                    if signal == 1:
                        if not trade_s and not trade_l:
                            ...
                            df.at[index, 'trade_type'] = 'not_in_trade'
                            trade_s = False
                        elif not trade_s and trade_l:
                            df.at[index, 'trade_type'] = 'long'
                    elif signal == -1:
...
def strat(start1,strat2,strat3,strat4):
    ...
    final['TP2'] = np.zeros(len(strat1))
    final['SL2'] = np.zeros(len(strat1))
    ...
    def master_strategy(df,win,small_win):
        ...
        while i<len(df):
            ...
            i+=win
        # if df['final_trade'][len(df)-1] == 'long':
        #     df['final_sig'] = -1
        # elif df['final_trade'][len(df)-1] == 'short':
        #     df['final_sig'] = 1
        # elif df['final_trade'][len(df)-1] == 'no_trade':
        #     df['final_sig'] = 0
    ...
    btc_4h['trade_type'] = master['trade_type']
    btc_4h['TP'] = master['TP']
    btc_4h['SL'] = master['SL']
...
if __name__ == "__main__":
    ...
    strat1,strat2,strat3,strat4 = process_data(r'C:\Users\harsh\Desktop\inter-iit\BTC_2019_2023_4h.csv',r'C:\Users\harsh\Desktop\inter-iit\btc_1D.csv')
    strat(strat1,strat2,strat3,strat4)
    perform_backtest(r'C:\Users\harsh\Desktop\inter-iit\btc_final.csv')